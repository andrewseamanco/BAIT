...
function getRequest() {
  ...
  fetch('/request' + queryString)
      ...
      .then((request) => {
        ...
        document.getElementById(DATECONTAINER).append(document.createTextNode(new Date(request.submissionDate).toLocaleDateString() + "  " + new Date(request.submissionDate).toLocaleTimeString()));
